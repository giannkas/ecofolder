...
def repeated_markings_in_conf():
  repeated_markings_in_conf.__doc__ = f"""
    {repeated_markings_in_conf.__name__} outputs the configurations leading to a marking and a '+' sign for those events that produce redundant markings (unless -del paramater is given).\n
    ...
    Options:
      -del --delete remove events whose postset has already an identical marking produced in its cone.
  ...
  for i in range(params):
    ...
    elif sys.argv[i] == "-del" or sys.argv[i] == "--delete":
      dlt = 1
