...
class Model:
  ...
  def attractors(self):
    ...
    ecofolder_call = os.path.join(self.current_dir, 'ecofolder')

    subprocess.call([ecofolder_call, self.filename, "-m", mci0])

    # 2. fetch final markings
    allfinals_call = os.path.join(self.current_dir, 'allfinals.pl')
    out = subprocess.check_output([allfinals_call, "-f", mci0], stderr=DEVNULL)
  ...
  def reach_any(self, m0, markings, num=0):
    ...
    ecofolder_call = os.path.join(self.current_dir, 'ecofolder')
    mci2dot_call = os.path.join(self.current_dir, 'mci2dot')

    subprocess.call([ecofolder_call, "-T", "reached_any", name, "-m", "tmp.mci"])
    ret = subprocess.call([mci2dot_call, "-reach", "reached_any", "tmp.mci"])
